...
#' }
render_ggplot_to_latex <- function(fig = NULL,
                           fig_path = NULL,
                           sanitize = TRUE,
                           width = 6.4,
                           height = 3.54,
  ...
  if (is.null(fig_path)) {
    cli::cli_abort("fig_path is NULL, don't know where to save the figure file.")
  }

  # Attempt to save the TikZ figure
  tryCatch({
    save_tikz(fig = fig, path = fig_path, sanitize = sanitize, width = width, height = height)
  }, error = function(e) {
  ...
  tryCatch({
    ltx_build_fig(fig_in = fig_path, fig_out = fig_out, title = title, caption = caption, label = label, copy_fig = copy_fig)
  }, error = function(e) {
